...
- The first time the site starts in a given worktree, it picks a free port in `44300`–`44399` and saves it with `git config --worktree worktreedevport.port <port>`.
...
- Any other tool — a client generator script, an AI agent, a teammate — gets the same port by running `git config --worktree --get worktreedevport.port`. No need to ask the running site what port it's on.
